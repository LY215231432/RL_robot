...
def _resolve_dataset_file_handler_class(dataset_file_handler_class_type: type | None) -> type:
    """Resolve the dataset exporter class only when recorder export is enabled."""

    if dataset_file_handler_class_type is not None:
        return dataset_file_handler_class_type

    try:
        from isaaclab.utils.datasets import HDF5DatasetFileHandler
    except ImportError as exc:
        raise ImportError(
            "Failed to import the default HDF5 dataset exporter. "
            "If you only want to run or visualize an environment, disable dataset export "
            "or remove recorder terms from the environment configuration."
        ) from exc
...
class RecorderManager(ManagerBase):
    ...
    def __init__(self, cfg: object, env: ManagerBasedEnv):
        ...
        if cfg.dataset_export_mode != DatasetExportMode.EXPORT_NONE:
            dataset_file_handler_class_type = _resolve_dataset_file_handler_class(cfg.dataset_file_handler_class_type)
            self._dataset_file_handler = dataset_file_handler_class_type()
        ...
        if cfg.dataset_export_mode == DatasetExportMode.EXPORT_SUCCEEDED_FAILED_IN_SEPARATE_FILES:
            dataset_file_handler_class_type = dataset_file_handler_class_type or _resolve_dataset_file_handler_class(
                cfg.dataset_file_handler_class_type
            )
            self._failed_episode_dataset_file_handler = dataset_file_handler_class_type()
