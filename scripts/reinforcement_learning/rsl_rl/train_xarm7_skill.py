...
"""Thin launcher that trains an xArm7 skill by skill name."""
...
def _print_skills():
    ...
    for skill_name, spec in sorted(SKILL_REGISTRY.items()):
        print(f"  - {skill_name}: {spec.train_task}")
        print(f"    {spec.description}")


def main():
    parser = argparse.ArgumentParser(description="Train an xArm7 skill through the skill registry.")
    ...
    target_script = Path(__file__).with_name("train.py")
    command = [sys.executable, str(target_script), "--task", spec.train_task, *passthrough]

    print(f"[INFO] Training skill '{args.skill}' using task '{spec.train_task}'.")
